...
;(function Preview_Elements_Group_Scope() {
  ...
  var Preview_Elements_Group = new Class({
    ...
    /**
     * If group opacity is set to 50% and elements opacity is A - 100% and B - 50%.
     *
     * Then data returned from application sets A i B elements with values converted over group opacity:
     * A: 50%
     * B: 25%
     *
     * We need to restore original values to manipulate only with group opacity without modifying every element inside
     * group which will slow-down and desynchronize an animation.
     */
    saveElementsOriginalOpacity: function() {
      if (this.mGroupElements.length > 0) {
        this.mGroupElements.forEach(function(pEl) {
          var elemOpacity = parseFloat(pEl.style.opacity || 1);
          var originalOpacity = elemOpacity / this.mGroupOpacity;

          pEl.style.opacity = originalOpacity;
        }.bind(this));
      }
    }
  });
...
;(function Preview_Elements_Multistate_Scope() {
  ...
  var Preview_Elements_Multistate = new Class({
        ...
        switch(pAnimation.show) {
          ...
          case 'flip':
            ...
            if(pAnimation.direction == 'y') {
              data.location = 'transform';
              data.start = 'rotateY(0deg)';
              data.stop = 'rotateY(180deg)';
            }
            else {
              data.location = 'transform';
              data.start = 'rotateX(0)';
              data.stop = 'rotateX(180deg)';
            }
      ...
      if(pAnimation && pAnimation.show) {
        switch(pAnimation.show) {
          ...
          case 'flip':
            ...
            if(pAnimation.direction == 'y') {
              data.location = 'transform';
              data.start = 'rotateY(180deg)';
              data.stop = 'rotateY(360deg)';
            }
            else {
              data.location = 'transform';
              data.start = 'rotateX(180deg)';
              data.stop = 'rotateX(360deg)';
            }
            break;
          case 'fade':
            data.location = 'opacity';
    ...
    changeState: function(pNewStateId, pAnimation) {
      ...
      if(this.mCurrentState == this.mStates.indexOf(next)) {
        return;
      }
      else {
    ...
    moveBy: function(pX, pY, pAnimation) {
      ...
      this.setPosition(pX ,pY, pAnimation);
    },

    moveTo: function(pX, pY, pAnimation) {
      if(this.clipped) {
        this.setPosition(pX ,pY, pAnimation);
      }
      else {
